...
import { select, stack, stackOffsetExpand, scaleUtc, extent, scaleLinear, area, scaleOrdinal, schemeCategory10, axisBottom, axisLeft } from 'd3';
...
const GlobalChangeLineGraph: React.StatelessComponent<ExternalProps> = props => {
    function createGraph(refNode: React.RefObject<SVGSVGElement>, data: CrnLocation[], state: BaseGraphState) {
        ...
        const x = scaleUtc().domain(extent(series.dates)).range([state.margins.left, state.width - state.margins.right]);
